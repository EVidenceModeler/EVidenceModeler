...
sub create_CDS_sequence {
    ...
    unless ($self->get_CDS_length()) {
        cluck "sorry, cannot create a CDS sequence for a CDS-lacking gene model.\n";
        return;
    }
...
sub create_protein_sequence {
    ...
    unless ($cds_sequence) {
        my $cds_length = $self->get_CDS_length();
        unless ($cds_length) {
            cluck "cannot translate a protein for a gene model lacking a CDS.\n";
            return;
        }
...
sub to_GTF_format {
    ...
    $gene_obj->trim_UTRs(); ## No use for UTR exons here.
    ...
    if ($gene_obj->{is_5prime_partial} || $gene_obj->{is_3prime_partial}) {
        print STDERR "Sorry, partial gene $TU_feat_name,$model_feat_name is partial.  No GTF created.\n";
        return (undef());
    }
    
    my $seqname = $preferences{seqname} || $gene_obj->{asmbl_id};
    my $source = $preferences{source} || "annotation";
    my $gene_id;
    if (my $token = $preferences{gene_id}) {
        $gene_id = $gene_obj->{$token};
    } else {
        $gene_id = $gene_obj->{TU_feat_name};
    }
    ...
    if (my $token = $preferences{model_id}) {
        ...
        $transcript_id = $gene_obj->{Model_feat_name};
    }
    ...
    for (my $i=0; $i < $num_exons; $i++) {
        ...
        if (my $cds_obj = $exon->get_CDS_obj()) {
            my ($lend, $rend) = sort {$a<=>$b} $cds_obj->get_coords();
            my $cds_exon_length = $rend - $lend + 1;
            
            
            if ($i == 0) { ## start codon:
                if ($cds_exon_length < 3) { 
                    print STDERR "Sorry, $TU_feat_name,$model_feat_name has a split start codon.  No gtf created.\n";
                    return (undef());
                }
                my ($start_codon_lend, $start_codon_rend);
                if ($orientation eq '+') {
                    ($start_codon_lend, $start_codon_rend) = ($lend, $lend + 2);
                } else {
                    ($start_codon_lend, $start_codon_rend) = ($rend-2, $rend);
                }
            ...
            my $frame = ($cds_length) % 3;
            
            
            if ($i == $#exons) { #stop codon
                if ($cds_exon_length < 3) {
                    print STDERR "Sorry, gene $TU_feat_name,$model_feat_name has a split stop codon.  No gtf created.\n";
                    return (undef());
                }
                my ($stop_codon_lend, $stop_codon_rend);
                if ($orientation eq '+') {
                    ($stop_codon_lend, $stop_codon_rend) = ($rend-2, $rend);
                    $rend -= 3; ## don't include stop codon in terminal CDS
                } else {
                    ($stop_codon_lend, $stop_codon_rend) = ($lend, $lend+2);
                    $lend += 3; ## don't include stop codon in terminal CDS
                }
...
sub to_GFF3_format {
    ...
    my $use_pub_locus = $preferences{use_pub_locus};
    ...
    $gff3_text .= "$asmbl_id\t$source\tgene\t$gene_lend\t$gene_rend\t.\t$strand\t.\tID=$gene_id;Name=$com_name$noteField$gene_alias\n";
    
    if ( ! ($gene_obj->{is_pseudogene} || $gene_obj->{gene_type} ne "protein-coding") ) {
        ...
        foreach my $gene_obj ($gene_obj_ref, $gene_obj_ref->get_additional_isoforms() ) {
            ...
            if ($use_pub_locus) {
                if (my $model_pub_locus = $gene_obj->{model_pub_locus}) {
                    $model_alias = ";Alias=$model_id";
                    $model_id = $model_pub_locus;
                }
            }
            
            $gff3_text .= "$asmbl_id\t$source\tmRNA\t$gene_lend\t$gene_rend\t.\t$strand\t.\tID=$model_id;Parent=$gene_id$model_alias\n";
            ...
            foreach my $exon ($gene_obj->get_exons()) {
                ...
                if (my $exon_feat_name = $exon->{feat_name}) {
                    $exon_ID_string = "ID=$exon_feat_name;";
                }
                $gff3_text .= "$asmbl_id\t$source\texon\t$exon_lend\t$exon_rend\t.\t$strand\t.\t${exon_ID_string}Parent=$model_id\n";
                ...
                if (my $cds_obj = $exon->get_CDS_obj()) {
                    ...
                    my $cds_ID_string = "ID=cds_of_$model_id";
                    my $partial_text = "";
                    ...
                    $gff3_text .= "$asmbl_id\t$source\tCDS\t$cds_lend\t$cds_rend\t.\t$strand\t$phase\t${cds_ID_string};Parent=$model_id$partial_text\n";
                }
            }
...
sub _get_cds_start_pos {
    ...
    my $orfPos = $longest_orf->{end5}; #init to first, longest orf.
    my $bestOrfPos;
    my @allOrfs = $longest_orf->orfs();
    for my $orfIndex (0..2) {
    ...
    if ($orfPos >3) {
        die "Error, longest ORF is found at position $orfPos, and should be between 1 and 3.  What's wrong with your gene?" . $self->toString();
    }
    $codon_start = $orfPos;
